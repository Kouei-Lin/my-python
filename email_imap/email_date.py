...
class EmailIMAPClient:
    ...
    def fetch_emails(self, mailbox_folder, since_date, until_date):
        since_date_str = since_date.strftime('%d-%b-%Y')
        until_date_str = until_date.strftime('%d-%b-%Y')
        search_criteria = f'(SINCE {since_date_str} BEFORE {until_date_str})'
...
class EmailDataSaver:
    ...
    def save_to_sqlite(self, db_filename, since_date, until_date):
        with sqlite3.connect(db_filename) as conn:
            ...
            for email in self.emails_content:
                ...
                if since_date <= email_date < until_date:
                    cursor.execute('''
