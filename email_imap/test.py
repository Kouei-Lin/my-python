...
class EmailContentExtractor:
    ...
    def parse_email_content(email_data):
        ...
        try:
            ...
            try:
                # Try parsing with an alternative format
                date = datetime.strptime(date_str, '%d %b %Y %I:%M %p %z').strftime('%Y-%m-%d %H:%M:%S')
            except ValueError:
    ...
    def extract_information_from_body(body):
        ...
        fields = ['Total size', 'Data read', 'Transferred', 'Duration']
...
def main():
    ...
    for email_uid in email_ids:
        try:
            ...
            # Convert size, read, and transferred to GB
            size_gb = extractor.convert_to_gb(size)
            read_gb = extractor.convert_to_gb(read)
            transferred_gb = extractor.convert_to_gb(transferred)
